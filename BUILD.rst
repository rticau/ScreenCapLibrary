Creating ScreenCapLibrary releases
============================

These instructions cover steps needed to create new releases of ScreenCapLibrary.
When applicable, the steps are listed as commands that can
...
3. Set version information to a shell variable to ease copy-pasting further
   ...
   For example, ``VERSION=1.0.1`` or ``VERSION=1.1a2``.
...
2. Generate a template for the release notes::
   ...
   When generating release notes for a preview release like ``1.0.1rc1``,
...
4. Check that release notes are generated correctly with::

      rst2html.py docs/ScreenCapLibrary-$VERSION.rst ScreenCapLibrary-$VERSION.html
  
5. Add, commit and push::

      git add docs/ScreenCapLibrary-$VERSION.rst
      git commit -m "Release notes for $VERSION" docs/ScreenCapLibrary-$VERSION.rst
      git push

6. Update later if necessary. Writing release notes is typically the biggest
...
1. Set version information in `<src/ScreenCapLibrary/version.py>`_::

      invoke set-version $VERSION

2. Commit and push changes::

      git commit -m "Updated version to $VERSION" src/ScreenCapLibrary/version.py
      git push
...
1. Generate keyword documentation::
      ...
      invoke kw-docs
      git commit -m "Generated docs for version $VERSION" docs/ScreenCapLibrary.html
      git push
...
2. Add short release notes to GitHub's `releases page
   <https://github.com/mihaiparvu/ScreenCapLibrary/releases>`_
...
5. Verify that project the page at `PyPI
   <https://pypi.org/project/robotframework-screencaplibrary/>`_
   looks good.
...
2. Set dev version based on the previous version::
      ...
      git commit -m "Back to dev version" src/ScreenCapLibrary/version.py
      git push
...
3. Close the `issue tracker milestone
   <https://github.com/mihaiparvu/ScreenCapLibrary/milestones>`_.
